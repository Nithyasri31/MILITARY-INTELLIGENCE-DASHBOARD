...
if filtered_df.empty:
    st.warning("No records found for the selected filters. Try removing one or more filters.")
else:
    st.dataframe(
    ...
    st.info(
        f"Showing first {min(rows_to_show, len(filtered_df))} rows out of {len(filtered_df)} filtered rows."
    )
...
csv = filtered_df.head(50000).to_csv(index=False)
